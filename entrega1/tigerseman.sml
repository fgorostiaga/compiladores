	...
	let val pilawhile= ref []
		...
		| trexp(OpExp({left, oper=EqOp, right}, nl)) =
			let
				...
				val _ = printtipo tyr
			in
				if (tiposIguales tyl tyr) andalso not (tyl=TNil) andalso not (tyl=TUnit) then {exp=(), ty=TInt RW}
		...
		| trexp(OpExp({left, oper, right}, nl)) = 
			let
				val {exp=_, ty=tyl} = trexp left
				val {exp=_, ty=tyr} = trexp right
				val tr = (case (tipoReal tyl) of TInt _ => TInt RW
				                                |t => t)
			in
				if tiposIguales tyl tyr then
					case oper of
						PlusOp => if tr=TInt RW then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| MinusOp => if tr=TInt RW then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| TimesOp => if tr=TInt RW then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| DivideOp => if tr=TInt RW then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| LtOp => if tr=TInt RW orelse tr=TString then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| LeOp => if tr=TInt RW orelse tr=TString then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| GtOp => if tr=TInt RW orelse tr=TString then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
						| GeOp => if tr=TInt RW orelse tr=TString then {exp=(),ty=TInt RW} else error("Error de tipos", nl)
		...
		| trexp(RecordExp({fields, typ}, nl)) =
			let
				...
				val _ = print ("CANT CS "^Int.toString(List.length cs))
		...
		| trexp(BreakExp nl) =
		    if (!pilawhile)=[] then error("Break fuera de While",nl) else {exp=(), ty=TUnit}
		...
		and trdec (venv, tenv) (VarDec ({name,escape,typ=NONE,init},pos)) = 
            let fun gettype(init) = let val {exp=_, ty=ti} = transExp(venv, tenv) init
                                       in (case ti of TNil => raise Fail "Nil not constrained"
                                                     | t => t) end
            in (tabRInserta (name,Var {ty = (gettype(init))} , venv), tenv, []) end
		| trdec (venv,tenv) (VarDec ({name,escape,typ=SOME s,init},pos)) =
            let fun gettype(s, init) = let val ti = (case tabBusca(s,tenv) of SOME t => tipoReal t
                                                                             |NONE => error("no existe el tipo", pos))
                                           val {exp=_, ty=td} = transExp(venv, tenv) init
                                           val _ = mychecktipo ti td pos
                                       in ti end
            in (tabRInserta (name,Var {ty = (gettype(s, init))} , venv), tenv, []) end
		...
		| trdec (venv,tenv) (TypeDec ts) =
          let 
            ...
            in
			(venv, addtotenv(ts, addtotenvnone(ts,tenv)), []) end
	in trexp end
